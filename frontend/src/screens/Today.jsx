...
function KickstartModal({ task, onClose }) {
...
export default function Today() {
  ...
  async function loadWhy(task) {
    ...
    setWhyLoading(true);
    try {
      const r = await api.whyNow(task.id);
      setWhy(r.reasoning);
    } catch {
      setWhy("Couldn't load reasoning right now.");
    } finally {
      setWhyLoading(false);
    }
  }
      ...
      {hero && (
        ...
        >
          ...
          <div className="relative">
            ...
            {/* Action CTAs — the "do it with you" differentiator */}
            <div className="flex flex-wrap items-center gap-2 mt-5">
              <button
                onClick={() => setKickstartTask(hero)}
                className="bg-primary text-on-primary-fixed font-bold px-4 py-2.5 rounded-xl flex items-center gap-2 hover:scale-102 active:scale-95 transition-transform shadow-lg shadow-primary/20"
              >
                <span className="material-symbols-outlined text-base">bolt</span>
                Kickstart this
              </button>
              <button
                onClick={() => loadWhy(hero)}
                disabled={whyLoading}
                className="px-4 py-2.5 rounded-xl text-primary border border-primary/30 hover:bg-primary/10 font-bold flex items-center gap-2 transition-colors disabled:opacity-50"
              >
                <span className="material-symbols-outlined text-base">
                  {whyLoading ? "progress_activity" : "psychology"}
                </span>
                {why ? "Hide" : "Why now?"}
              </button>
            </div>
            ...
            {why && (
              <div className="mt-3 text-on-surface-variant text-body-md bg-surface-container-low border border-primary/20 rounded-xl p-unit-md flex gap-2 animate-fade-in-up">
        ...
        {rest.map((t) => (
          ...
          >
            ...
            <div className="flex items-center gap-2 shrink-0 ml-3">
              ...
              <button
                onClick={() => setKickstartTask(t)}
                className="opacity-0 group-hover:opacity-100 w-7 h-7 rounded-lg flex items-center justify-center text-primary hover:bg-primary/10 transition-all"
