...
export default function Login() {
  ...
  // Set up the OAuth popup token client. Our own button triggers it, so we keep
  // full control of the look (dark, no inline email) AND a reliable sign-in:
  // the popup returns an access token, which the backend verifies via userinfo.
  useEffect(() => {
    ...
    <div className="bg-background text-on-background min-h-screen flex items-center justify-center overflow-hidden relative">
      ...
      <main className="w-full max-w-[440px] px-margin-mobile relative z-10">
        ...
        <div className="glass-card rounded-2xl p-unit-lg shadow-2xl login-enter">
          ...
          {/* Social — our own dark button triggers Google's popup token flow:
              reliable sign-in, matches the theme, and never shows an email inline. */}
          <button
